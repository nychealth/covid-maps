...
// vax neighborhood min and maxes
var vax1PlusMin;
var vax1PlusMax;
var vaxFullyMin;
var vaxFullyMax;
...
// pulls in vax by boro data
d3.csv("https://raw.githubusercontent.com/nychealth/covid-vaccine-data/main/people/coverage-summary-allages.csv").then(function(data) {
    boroVax = data;
...
var mostRecent;
var dataNow
// d3 code pulls in Transmission data. for this to work, the D3 library must be referenced in the HTML
d3.csv("https://gist.githubusercontent.com/mmontesanonyc/2fdc46bcac05e56cac382524717e4d35/raw/2a9415cc64fb5e2ed3b1b8f513c4677c15619912/sevenday_caserates.csv").then(function (data) {
    trendData = data;
...
d3.csv("https://raw.githubusercontent.com/nychealth/covid-vaccine-data/main/people/coverage-by-modzcta-allages.csv").then(function(data){
    ...
    vax1MedArray.sort(function (a, b) { return a - b }); // sorts in ascending order
    ...
    vaxFullyMedArray.sort(function (a, b) { return a - b }); // sorts in ascending order
...
// this lets us sort a json more easily. we can use this for better min/max/median calculations
function byField(fieldName){
  ...
  // This function changes the tick chart between 1PLUS and FULLY based on button input (x)
function vax(x) {
    ...
    if (x == 1) {
        ...
        tickSpec2.layer[0].encoding.tooltip[1].title = "Percent with at least 1 dose";
        tickSpec2.layer[1].encoding.tooltip[1].field = "PERC_1PLUS";
        tickSpec2.layer[1].encoding.tooltip[1].title = "Percent with at least 1 dose";
        ...
        document.getElementById('metrictext').innerHTML = "have had at least 1 dose."
...
// This function runs on button click and changes the variable "label" for the line chart
...
function changeNeighborhood(zipCode) {
    ...
    zipVaxData = vaxData.filter(neighborhood => neighborhood.MODZCTA == zipString); // filters vax data to just selected neighb
    // adding zip and citywide vax values to summary table
    document.getElementById('pv').innerHTML = Number(zipVaxData[0].COUNT_1PLUS_CUMULATIVE).toLocaleString("en-US");
    document.getElementById('pfv').innerHTML = Number(zipVaxData[0].COUNT_FULLY_CUMULATIVE).toLocaleString("en-US");
...
var uhfList = [{
  ...
  },
  {
    ...
    "namezip": "Central Harlem - Morningside Heights (10026,10027,10030,10037,10115)",
